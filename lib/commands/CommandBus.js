...
class CommandBus {
    ...
    add(type, command){
        ...
        /*
         * Because we are using EventHub we get
         * pattern matching on the subscription
         * level, we don't need to register
         * execute.
         */
        this.eventBus.subscribe(type, (event)=>{
            // this.execute(event);
            this.doExecute(command, event);
        });
    }
...
class CommandSet extends Array {
    set(value){
        ...
        value.map((c)=>{
            if(c){
                // console.log('Add command', c.constructor.name);
                this.push(c);
            }
        });
        return this;
